...
export default function PerformanceMetricCard({ metric, icon: Icon, color, format = 'number', loading }) {
  ...
  const displayValue =
    format === 'currency' ? formatKSh(value) :
  ...
  return (
    <div className="rounded-xl border border-border bg-background p-5 shadow-sm transition-shadow hover:shadow-md">
      ...
      <p className="mt-4 text-2xl font-bold tracking-tight text-foreground">{displayValue}</p>
      <p className="mt-1 text-sm text-muted-foreground">{label}</p>
      <p className="mt-2 text-xs text-muted-foreground/60">{isPositive ? '▲' : '▼'} vs last month</p>
    </div>
  )
}
